...
README.md section "6. Planned continuous feedback and batch fine-tuning loop"
is the current plan for learning from human SQLite feedback. Collect continuously,
train in separate versioned batches (initial proposal: daily check, about 100 new
usable labels), evaluate against deployed v1, and require manual promotion at
first. Preserve correction/provenance history in snapshots, prevent split leakage,
include older examples, and retain rollback checkpoints. The orchestrator and
schedule are not implemented; adding this plan does not authorize automatic
training or checkpoint promotion. Consult the README for the full workflow.
